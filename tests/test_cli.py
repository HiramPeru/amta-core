...
def test_cli_query_both_direction(tmp_path: Path) -> None:
    runner.invoke(app, ["init", "--path", str(tmp_path)])
    nodes_dir = tmp_path / ".amta" / "nodes"
...
""".lstrip(),
    ...
    assert result.exit_code == 0
    assert "QUERY MOD-001 direction=both depth=1" in result.output
    assert "direction=upstream" in result.output
    assert "direction=downstream" in result.output
    assert "node=DEC-001" in result.output
    assert "node=TSK-001" in result.output
    assert "edge=DEC-001->MOD-001" in result.output
    assert "edge=MOD-001->TSK-001" in result.output
